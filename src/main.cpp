...
const unsigned long REFRESH_INTERVAL =
    2 * 60 *
    1000; // TODO: 2 minutes currently, use more infrequent updates in final
...
const char *TASKS_ENDPOINT =
    "/api/calendar/tasks"; // TODO: update when tasks are exposed
...
bool fetchTasksData() {
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (changed) {
      ...
      ui_journal_update_entries(payload);
    } else {
...
void updateHeader() {
  if (currentPage == PAGE_CALENDAR) {
    ui_journal_update_header(wifiConnected);
  } else {
    ui_journal_update_header(wifiConnected);
  }
